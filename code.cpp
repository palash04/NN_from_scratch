...
struct Node{
    ...
    vector<Node*> parents;
    int indegree;       // for computing forward topological ordering
    int outdegree;      // for computing reverse topological ordering
    int nodeNum;
    double gradient;
    Node() {
...
class Graph{
    ...
    int label;
public:
    void linear(int numOfInputNodes, int numOfOutputNodes, vector<Node*> input_hidden_vec, bool bias, bool first_layer) {
        ...
        // return hidden/output nodes
        pair<vector<Node*>, vector<Node*>> p = {input_vec, hidden_vec};
        output_nodes.clear();
...
int main() {
    Graph g;
    int n;
    cout << "Enter the number of layers (including input and output layers): ";
    cin >> n;

    vector<int> layers_info(n);
    for (int i=0;i<n;i++) {
        cout << "\nEnter number of nodes in layer " << i + 1 << " : ";
        cin>>layers_info[i];
    }
    ...
    for (int i=0;i<n-1;i++) {
        if (i == 0)
            g.linear(layers_info[i],layers_info[i+1], {}, false, true);
        else
            g.linear(layers_info[i],layers_info[i+1], {}, false, false);
    }
